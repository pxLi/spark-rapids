...
if [ -d "$LOCAL_JAR_PATH" ]; then
    if [[ $TEST_MODE == "ALL" || $TEST_MODE == "IT_ONLY" ]]; then
        ...
        TEST_PARALLEL=0 LOCAL_JAR_PATH=$LOCAL_JAR_PATH bash $LOCAL_JAR_PATH/integration_tests/run_pyspark_from_build.sh  --runtime_env="databricks" --test_type=$TEST_TYPE -k window_function_test
    ...
    if [[ $TEST_MODE == "ALL" || $TEST_MODE == "IT_ONLY" ]]; then
        ...
        TEST_PARALLEL=0 bash /home/ubuntu/spark-rapids/integration_tests/run_pyspark_from_build.sh --runtime_env="databricks" --test_type=$TEST_TYPE -k window_function_test
