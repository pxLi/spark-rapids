...
mvn_verify() {
    ...
    [[ $BUILD_MAINTENANCE_VERSION_SNAPSHOTS == "true" ]] && $MVN_INSTALL_CMD -DskipTests -Dbuildver=314

    # don't skip tests
    env -u SPARK_HOME $MVN_CMD -U -B $MVN_URM_MIRROR -Dbuildver=320 clean install $MVN_BUILD_ARGS -Dpytest.TEST_TAGS='' -pl '!tools'
    # enable UTF-8 for regular expression tests
    env -u SPARK_HOME LC_ALL="en_US.UTF-8" $MVN_CMD $MVN_URM_MIRROR -Dbuildver=320 test $MVN_BUILD_ARGS -Dpytest.TEST_TAGS='' -pl '!tools' -DwildcardSuites=com.nvidia.spark.rapids.ConditionalsSuite,com.nvidia.spark.rapids.RegularExpressionSuite,com.nvidia.spark.rapids.RegularExpressionTranspilerSuite
    $MVN_INSTALL_CMD -DskipTests -Dbuildver=321
    $MVN_INSTALL_CMD -DskipTests -Dbuildver=322
    env -u SPARK_HOME $MVN_CMD -U -B $MVN_URM_MIRROR -Dbuildver=330 clean install MVN_BUILD_ARGS -Dpytest.TEST_TAGS='' -pl '!tools'
